...
export default function FormUsers({ params }) {
  ...
  const { season_id, tv_series_id, id } = params
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    try {
      ...
      const response = await axios.post(`/api/episodes/${id}`, formData, config)
      ...
      if (response.status !== 200) {
        ...
        return navigate(`/studio/content/tvseries/${tv_series_id}`)
      }
    } catch (error) {
      ...
      <form onSubmit={onSubmit} className={'flex bg-[#212129] '}>
        <div className=" w-full   p-3  rounded-lg">
          <div className="flex justify-between">
            <h1 className="font-bold text-2xl text-white">Edit Episode</h1>
            <Code className="mr-4">
              ID {season_id}-{tv_series_id}-{id}
            </Code>
          </div>
          <hr className="mb-3" />
          <input
          ...
          <input
            name={'episodes_id'}
            type="text"
            value={Data['episode.episodes_id']}
            hidden
          />
            ...
            <div className="grow">
              ...
              <div className="flex flex-wrap -mx-3 mb-3">
                <div className="w-full md:w-1/2 px-3">
                  <Input
                    isRequired
                    defaultValue={Data['episode.episode_number']}
                    name={'episode_number'}
                    type="number"
                    variant={variant}
